...
<div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4">
        <div>
            <h1 class="mb-1 text-primary">İletişim Mesajları</h1>
        </div>
    </div>
        ...
        <div class="col-md-6 col-xl-3">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    ...
                    <div class="text-muted small">Tüm iletişim mesajları</div>
                </div>
            </div>
        </div>
        <div class="col-md-6 col-xl-3">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    <span class="text-muted small text-uppercase">Okunmadı</span>
                    <div class="display-6 fw-semibold text-warning">{{ $stats['unread'] }}</div>
                    <div class="text-muted small">İlgi bekliyor</div>
                </div>
            </div>
        </div>
        <div class="col-md-6 col-xl-3">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    <span class="text-muted small text-uppercase">Okundu</span>
                    <div class="display-6 fw-semibold text-success">{{ $stats['read'] }}</div>
                    <div class="text-muted small">İncelendi</div>
                </div>
            </div>
        </div>
        <div class="col-md-6 col-xl-3">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    <span class="text-muted small text-uppercase">Yanıtlandı</span>
                    <div class="display-6 fw-semibold text-primary">{{ $stats['replied'] }}</div>
                    <div class="text-muted small">Panelden yanıtlandı</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card shadow-sm mb-4 contact-message-filter-card">
        <div class="card-header bg-dark text-white">
            <h5 class="mb-0">Mesajları Filtrele</h5>
        </div>
        <div class="card-body">
            <form method="GET" action="{{ route('admin.content.contact-messages.index') }}">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="search" value="{{ $filters['search'] }}" class="form-control" placeholder="Ad, e-posta, konu, mesaj">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Durum</label>
                        <select name="status" class="form-select">
                            <option value="">Tüm durumlar</option>
                            <option value="unread" {{ $filters['status'] === 'unread' ? 'selected' : '' }}>Okunmadı</option>
                            <option value="read" {{ $filters['status'] === 'read' ? 'selected' : '' }}>Okundu</option>
                            <option value="replied" {{ $filters['status'] === 'replied' ? 'selected' : '' }}>Yanıtlandı</option>
                            <option value="not_replied" {{ $filters['status'] === 'not_replied' ? 'selected' : '' }}>Yanıtlanmadı</option>
                        </select>
                    </div>
                    <div class="col-md-3 d-flex align-items-end gap-2">
                        <button type="submit" class="ui-btn ui-btn-primary w-100">Uygula</button>
                        <a href="{{ route('admin.content.contact-messages.index') }}" class="ui-btn ui-btn-neutral w-100">Sıfırla</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-dark text-white d-flex align-items-center justify-content-between">
            <h5 class="mb-0">Mesaj Listesi</h5>
            <span class="badge bg-light text-dark">{{ $messages->total() }} sonuç</span>
        </div>
                ...
                <table class="table align-middle mb-0">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th style="width: 18%;">Konu</th>
                            <th>Mesaj</th>
                            <th style="width: 10%;">Durum</th>
                            <th style="width: 12%;">Gönderim</th>
                            <th style="width: 16%;" class="text-end">İşlemler</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($messages as $message)
                            @php($isÜyeSender = $message->user_id || in_array(strtolower($message->email), $memberEmails, true))
                            <tr class="{{ $message->read_at ? '' : 'contact-message-row-unread' }}">
                                ...
                                <td>
                                    ...
                                    @if($message->replied_at)
                                        <span class="badge bg-primary mt-1">Yanıtlandı</span>
                                    @endif
                                </td>
                                <td>
                                    ...
                                    <div class="d-flex flex-wrap justify-content-end gap-2">
                                        ...
                                        @if($message->read_at)
                                            <form method="POST" action="{{ route('admin.content.contact-messages.unread', $message) }}">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="ui-btn ui-btn-warning ui-btn-sm">Okunmadı</button>
                                            </form>
                                        @else
                                            <form method="POST" action="{{ route('admin.content.contact-messages.read', $message) }}">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="ui-btn ui-btn-success ui-btn-sm">Okundu</button>
                                            </form>
                                        @endif
                                        <form method="POST" action="{{ route('admin.content.contact-messages.destroy', $message) }}" onsubmit="return confirm('Bu mesaj silinsin mi?');">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="modal-content">
                    ...
                    <div class="contact-message-modal-heading">
                        ...
                        <div>
                            <div class="contact-message-title-row">
                                <h5 class="modal-title">{{ $message->subject ?: 'Konu yok' }}</h5>
                                <div class="contact-message-status-stack">
                                    <span class="contact-message-status-pill {{ $isÜyeSender ? 'is-member' : '' }}">
                                        <i class="fa-solid {{ $isÜyeSender ? 'fa-user-check' : 'fa-user' }}" aria-hidden="true"></i>
                                        {{ $isÜyeSender ? 'Üye' : 'Misafir' }}
                                    </span>
                ...
                <div class="modal-body">
                    <div class="contact-message-body-shell">
                        <span class="contact-message-body-label">Mesaj</span>
                    ...
                    <form method="POST" action="{{ route('admin.content.contact-messages.reply', $message) }}" class="contact-message-reply-shell">
                        ...
                        <div class="d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-2 mt-3">
                            <span class="text-muted small">Bu yanıt e-posta ile gönderilecek: {{ $message->email }}.</span>
                            <button type="submit" class="ui-btn ui-btn-primary">
                                <i class="fa-regular fa-paper-plane" aria-hidden="true"></i>
                                <span>{{ $message->replied_at ? 'Güncellenmiş Yanıtı Gönder' : 'Yanıt Gönder' }}</span>
                            </button>
                        </div>
                    </form>
                </div>
                <div class="modal-footer justify-content-between">
                    <span class="contact-message-submitted">Gönderim {{ $message->created_at->format('d M Y H:i') }}</span>
